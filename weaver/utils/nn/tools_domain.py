...
## train classification into a total loss --> best training epoch decided on the loss function
def train_classdomain(model, loss_func, opt, scheduler, train_loader, dev, epoch, steps_per_epoch=None, grad_scaler=None, tb_helper=None):
    ...
    label_cat_counter = Counter()
    total_cat_correct, total_domain_correct, sum_sqr_err = 0, 0 ,0
    inputs, label_cat, label_domain, model_output, model_output_cat, model_output_reg, model_output_domain = None, None, None, None, None, None, None
    ...
    with tqdm.tqdm(train_loader) as tq:
        ...
        for X, y_cat, _, y_domain, _, y_cat_check, y_domain_check in tq:
            ...
            ### Number of samples in the batch
            # num_cat_examples = max(label_cat.shape[0])
            num_cat_examples = label_cat.shape[0]
            # print (True if label_domain.shape else False)
            # num_domain_examples = label_domain.shape[0]
            num_domain_examples = label_domain.shape[0] if label_domain.shape else 0
            ...
            for idx, (k,v) in enumerate(y_domain_check.items()):
                if num_domains == 1:
                    ...
                    # print (label_domain)
                    # print (index_domain[k])
                    index_domain[k] = label_domain_check.nonzero()
                    # print (index_domain[k])
                    # print (idx, (k,v), index_domain[k])
                    # if index_domain[k]:
                    label_domain_np = label_domain[index_domain[k]].squeeze().cpu().numpy().astype(dtype=np.int32)
                    if np.iterable(label_domain_np):
                        label_domain_counter[idx].update(label_domain_np)
...
## evaluate classification + regression task
def evaluate_classdomain(model, test_loader, dev, epoch, for_training=True, loss_func=None, steps_per_epoch=None, tb_helper=None,
                      eval_cat_metrics=['roc_auc_score', 'roc_auc_score_matrix', 'confusion_matrix']):
    ...
    with torch.no_grad():
        with tqdm.tqdm(test_loader) as tq:
            ...
            # for X, y_cat, y_domain, Z, y_cat_check, y_domain_check in tq:
                ...
                # print (Z.shape)
                # print (y_cat_check.shape)
                # print (y_domain_check.shape)
                ### input features for the model
                inputs = [X[k].to(dev,non_blocking=True) for k in data_config.input_names]
                ...
                ### store truth labels for classification as well as observers
                for k, v in y_cat.items():
                    if not for_training:
                        labels_cat[k].append(_flatten_label(v,None).cpu().numpy().astype(dtype=np.int32))
                    else:
...
def evaluate_onnx_classdomain(model_path, test_loader,
                           eval_cat_metrics=['roc_auc_score', 'roc_auc_score_matrix', 'confusion_matrix']):
    ...
    total_loss, num_batches, total_cat_correct, total_domain_correct, sum_sqr_err, count_cat, count_domain = 0, 0, 0, 0, 0, 0, 0
    labels_cat, labels_domain, observers = defaultdict(list), defaultdict(list), defaultdict(list)
    scores_cat, scores_domain = [], []
    scores_domain  = defaultdict(list) 
    pred_cat, pred_domain = None, None
    inputs, label_cat, label_domain, model_output, model_output_cat, model_output_domain  = None, None, None, None, None , None
    indexes_cat = []
    indexes_domain = defaultdict(list) 
    index_offset = 0
    ...
    with tqdm.tqdm(test_loader) as tq:
        for X, y_cat, y_domain, Z, y_cat_check, y_domain_check in tq:
            ...
            ### define truth labels for classification
            indexes_cat.append((index_offset+index_cat).cpu().numpy().astype(dtype=np.int32))
            for k, v in y_cat.items():
                labels_cat[k].append(_flatten_label(v,None).cpu().numpy().astype(dtype=np.int32))              
            for k, v in Z.items():                
